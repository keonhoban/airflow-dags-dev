...
class TrainSkippableError(RuntimeError):
    """학습 자체가 불가능한 조건(표본/클래스 부족 등)에서 스킵 판단용 에러"""
    pass
...
def _make_label(df: pd.DataFrame, q: int = 3) -> pd.Series:
    """
    proxy label (q-class)
    - 값이 모두 같아도 rank 기반 qcut이면 분산 가능
    """
    ...
    r = x.rank(method="first")  # 값이 모두 같아도 1..N으로 분산

    if len(df) < 2:
        raise TrainSkippableError(f"label 생성 실패: rows={len(df)} (최소 2 필요)")

    # 3-class 시도 → 실패하면 2-class fallback
    if q >= 3 and len(df) >= 3:
        try:
...
def train_model(C, max_iter, feature_uri=None, fs_version=None, schema_hash=None):
    ...
    if not feature_uri:
        raise ValueError("feature_uri is required for full integration")
    ...
    logger.info("[TRAIN] using feature_uri=%s rows=%d classes=%s", feature_uri, len(df), uniq)

    # stratify 가능한 경우만 stratify
    try:
