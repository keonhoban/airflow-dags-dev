...
def build_config_pbtxt(model: str, in_name: str, out_prob: str, out_label: str, n_features: int, n_classes: int) -> str:
    """
    ✅ ONNX 입출력 이름 + 차원 기반으로 config.pbtxt 생성
    """
    return f'''name: "{model}"
...
def _parse_output_names(raw: str):
    xs = [x.strip() for x in (raw or "").split(",") if x.strip()]
    return xs
...
def materialize(
    ...
    **_,
    ...
    ✅ shadow: run_id -> repo/<model>/<timestamp_version>/  (레지스트리 없이도 배포 검증 가능)

    shadow=True일 때:
      - 기본은 같은 model_name을 사용(=dev에서 E2E 확인 목적)
      - 원하면 triton_model_name_shadow 변수를 쓰도록 옵션 제공
    """
    base_model = cfg("triton_model_name", required=True)
    model = base_model

    if shadow:
        # (선택) shadow 모델명을 분리하고 싶으면 Variable/ENV로 지정
        model = cfg("triton_model_name_shadow", base_model)
    ...
    if run_id:
        ...
        deploy_version = int(datetime.now().strftime("%Y%m%d%H%M%S"))  # ✅ Triton version dir must be int
        mode = "shadow"
    else:
    ...
    # ---- MLflow params에서 n_features/n_classes/onnx io 읽기 ----
    ...
    # skl2onnx zipmap False의 흔한 출력 이름: ["label", "probabilities"] or ["output_label","output_probability"]
    # 확실히 하려고: label/probabilities 둘 다 찾고 없으면 순서 기반 fallback
    out_label = "label"
    ...
    if outs:
        ...
        for cand in outs:
            if "prob" in cand.lower():
                out_prob = cand
                break
        # 둘 다 못 찾으면 2개일 때 [0],[1]을 사용
        if len(outs) >= 2:
            if out_label not in outs and out_prob not in outs:
                out_label, out_prob = outs[0], outs[1]

    # 안전망: train 쪽이 param 기록을 못 했으면 여기서라도 멈추는 게 낫습니다
    if n_features <= 0 or n_classes <= 0:
        raise RuntimeError(f"[Triton] invalid n_features/n_classes from run params: n_features={n_features}, n_classes={n_classes}")
    ...
    ti.xcom_push(key="onnx_out_prob", value=out_prob)
    ti.xcom_push(key="onnx_out_label", value=out_label)

    log.info("[W6] materialize OK mode=%s model=%s alias=@%s version=%s run_id=%s dst=%s", mode, model, alias, deploy_version, chosen_run_id, dst)
    log.info("[W6] config.pbtxt updated path=%s n_features=%d n_classes=%d in=%s out_prob=%s out_label=%s", config_path, n_features, n_classes, in_name, out_prob, out_label)
...
def triton_load(ti, **_):
    ...
    mode = ti.xcom_pull(task_ids="materialize_repo", key="deploy_mode")
    triton = build_triton_http_url()

    log.info("[W6] triton_load model=%s mode=%s alias=@%s triton=%s", model, mode, alias, triton)
...
def triton_infer_smoke(ti, **_):
    ...
    if n_features <= 0:
        raise RuntimeError("[W6] smoke: missing n_features")
...
def rollback_minimal(ti, **_):
    ...
    try:
        r = requests.post(f"{triton}/v2/repository/models/{model}/load", timeout=10)
        log.warning("[W6][ROLLBACK] reload status=%s body=%s", r.status_code, r.text[:300])
    except Exception as e:
        log.warning("[W6][ROLLBACK] reload failed: %s", e)
