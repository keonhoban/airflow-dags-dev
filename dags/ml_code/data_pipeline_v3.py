# dags/ml_code/data_pipeline_v3.py
...
def _ensure_region_env(default_region: str = "ap-northeast-2") -> str:
    """
    boto3는 region을 환경변수/AWS config에서 찾는데,
    현재 건호님 케이스처럼 credentials에 region만 있고 env에 없으면 None이 뜰 수 있습니다.
    최소한 env에 AWS_DEFAULT_REGION을 박아줍니다.
    """
    r = os.getenv("AWS_DEFAULT_REGION") or os.getenv("AWS_REGION")
    if not r:
...
def validate_data(raw_path: str, pipeline_name: str, ti):
    ...
    logger.info(
        "[DP] pipeline=%s step=validate_data status=%s rows=%d null_rate=%.4f",
        pipeline_name,
...
def build_features(
    ...
    """
    - schema 파일을 읽고
    - feature_set 컬럼 순서대로 CSV 생성
    - 결과는 XCom에 적재 (store_features가 저장)
    """
    ...
    logger.info(
        "[FS] build_features OK set=%s rows=%d schema_hash=%s base=%s",
        feature_set,
        len(feature_rows),
        schema_hash,
        feature_base,
...
def store_features(
    ...
    if not features_csv:
        raise ValueError("[FS] features_csv 누락 (build_features가 정상 생성했는지 확인 필요)")
...
def summarize_run(pipeline_name: str, feature_set: str, ti):
    logger.info(
        "[FS] summarize pipeline=%s feature_set=%s raw=%s out_prefix=%s version=%s uri=%s",
        pipeline_name,
        feature_set,
