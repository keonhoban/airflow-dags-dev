...
def _make_label(df: pd.DataFrame, q: int = 3) -> pd.Series:
    ...
    if len(df) < 2:
        raise TrainSkippableError(f"rows={len(df)} (min 2)")
...
def _export_onnx_and_log(clf, n_features: int):
    ...
    try:
        ...
        log.info("[ONNX] io input=%s output=%s", in_name, out_names)
    except Exception as e:
        log.warning("[ONNX] io extract failed: %s", e)
...
def train_model_and_log(ti) -> tuple[float, str, int]:
    ...
    if missing:
        raise TrainSkippableError(f"feature 컬럼 누락: {missing}")
    ...
    if len(uniq) < 2:
        raise TrainSkippableError(f"class 부족: unique={uniq}")
    ...
    with mlflow.start_run(experiment_id=exp_id) as run:
        ...
        log.info("[TRAIN] acc=%.4f run_id=%s feature_uri=%s", acc, run_id, feature_uri)
        return float(acc), run_id, int(X.shape[1])
