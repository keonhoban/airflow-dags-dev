...
from .build import build_features as _build
from .store import store_features as _store
...
def task_extract_raw_data(**context):
    ...
    logger.info("[DP] head_object bucket=%s key=%s", bucket, key)
...
def task_validate_data(**context):
    ...
    if len(lines) <= 1:
        raise ValueError("data invalid: empty or header-only")

    ti.xcom_push(key="dp_valid", value=True)
    ti.xcom_push(key="dp_lines", value=len(lines))
    logger.info("[DP] validate OK lines=%d", len(lines))
...
def task_summarize_run(**context):
    ...
    logger.info(
        "[SUMMARY] pipeline=%s raw=%s version=%s uri=%s",
        cfg.pipeline_name,
